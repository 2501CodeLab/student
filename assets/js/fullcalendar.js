	...
	Calendar.mixin({
		...
		updateToolbarButtons: function() {
			...
			this.toolbarsManager.proxyCall(
				(todayInfo.isValid && !isDateWithinRange(now, view.currentRange)) ? 'enableButton' : 'disableButton', 'today');
			...
			if (events) {
				if ($.isFunction(events)) {
					...
					events.call(t, // this, the Calendar object
						rangeStart.clone(), rangeEnd.clone(), t.opt('timezone'),
						function(events) {
							callback(events);
							t.popLoading();
						});
